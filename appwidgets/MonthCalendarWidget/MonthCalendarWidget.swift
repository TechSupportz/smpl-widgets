...
#Preview(as: .systemSmall) {
	...
	MonthCalendarEntry(date: Calendar.current.date(byAdding: .day, value: -1, to: .now) ?? .now)
    MonthCalendarEntry(date: .now)
	MonthCalendarEntry(date: Calendar.current.date(byAdding: .month, value: 1, to: .now) ?? .now)
}
